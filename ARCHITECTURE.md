...
Di luar kedua fitur itu, larangan §5.2 berlaku utuh — dan pada beberapa pemicu ia bahkan lebih ketat daripada sekadar "tidak di-SELECT". Pada rekap resep harian (042) `reg_periksa` dan `pasien` mula-mula **tidak ikut dalam query sama sekali** — bukan "dibaca lalu tidak ditampilkan", melainkan tidak ada jalan apa pun dari query itu menuju seorang pasien. **`migrations/049` melonggarkannya, dan itu harus dibaca sadar-sadar**: pecahan per cara bayar menuntut `kd_pj`, yang cuma hidup di `reg_periksa`. `pasien` tetap tidak disebut dan tidak satu pun kolom identitas ikut keluar; yang menggantikan pagar lama adalah bentuk kodenya (query tersendiri, daftar SELECT cuma penjamin plus dua angka) — sama seperti 046 dan 047. Pada rekap asesmen awal (044) tanda vital dibaca sebagai `TRIM(p.td) <> ''` — sebuah **boolean**, sehingga query-nya mengetahui tekanan darah sudah dicatat tanpa pernah mengetahui berapa angkanya.

**Pada rekap bulanan administrasi (047) pagarnya berpindah TEMPAT, dan itu perlu dibaca sebelum menyentuh query-nya.** Ia satu-satunya rekap yang berangkat DARI `reg_periksa` — tabel yang memuat `no_rkm_medis`, `kd_poli`, `p_jawab`, dan `almt_pj` sekaligus — sehingga bentuk pagar "tabelnya memang tidak disebut" (042) tidak tersedia sama sekali di sana. Yang menggantikannya tiga bentuk kode, bukan disiplin pembacanya: identitas hanya pernah muncul sebagai argumen `COUNT(DISTINCT ...)` dan `GROUP BY` di dalam tabel turunan yang hasilnya langsung di-`COUNT` lagi; `kd_poli` **tidak dibaca sama sekali**, karena pecahan per poli pada bulan sepi adalah keterangan tentang orang tertentu; dan kelengkapan berkas dibaca lewat **`EXISTS`, bukan JOIN** — bentuk yang menghasilkan boolean, sehingga query mengetahui SOAPIE sudah diisi tanpa pernah mengetahui apa isinya. Empat tabel yang disentuhnya (`pemeriksaan_ralan`, `diagnosa_pasien`, `resume_pasien`, `penilaian_awal_keperawatan_ralan`) semuanya memuat rekam medis telanjang, dan `EXISTS` memang tidak punya tempat untuk membawanya keluar.

**Cara membuktikannya bukan dengan membaca SQL, melainkan memeriksa `Object.keys()` baris hasilnya.** Skrip `dryrun:penjualan`, `dryrun:resep`, `dryrun:penilaian`, `dryrun:bulanan`, dan `dryrun:adm-bulanan` melakukan itu dan **keluar dengan kode 1** bila ada kolom terlarang yang menyelinap masuk. Semuanya dibuktikan MENGGIGIT dengan menambahkan kolom pasien ke daftar SELECT dengan sengaja, bukan diasumsikan.
...
**(d) Fase PENAUTAN dijaga terpisah, dan itu perbaikan 14 Agustus 2026.** Ketiga hal di atas dulu baru terbentang SESUDAH `await initWaClient()`, jadi proses yang menggantung DI DALAM penautan tidak berdenyut dan tidak diawasi siapa pun — dan yang menyudahinya, `protocolTimeout` puppeteer, melempar ke `main().catch()` → `process.exit(1)` tanpa `shutdown()`, sehingga Chromium mati mendadak di tengah menulis state sesi dan start berikutnya mewarisi kerusakannya. Sekarang `heartbeat` dan `session-watchdog` dipasang sebelum penautan, dengan cabang `menautkan` berbatas **180 detik** (lebih pendek daripada `protocolTimeout` 300 detik, supaya keduanya tidak berlomba) yang keluar lewat `shutdown()`. Keputusannya fungsi murni di `core/watchdog.ts`, karena keadaan yang paling perlu dipatok justru yang paling mustahil dibuat sengaja di sistem hidup. Dua hal yang menempel: **status tidak dibaca selama penautan** (baris `wa_session` masih milik proses sebelumnya, dan `ready` yang basi mengirim watchdog memeriksa kesehatan klien yang belum jadi), dan **`session-command` sengaja TIDAK ikut naik** — ia mengosongkan kolom perintah sebelum bertindak, jadi menaikkannya berarti perintah dashboard ditelan diam-diam justru saat paling dibutuhkan.
