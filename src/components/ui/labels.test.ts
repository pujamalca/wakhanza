...
import { kodePemicuDariMigrasi } from '../../../scripts/checks/migrasiTemplate';
...
/**
 * Parsernya PINDAH ke `scripts/checks/migrasiTemplate.ts` supaya uji ini dan
 * `npm run preflight` membaca migrasi lewat satu penurunan yang sama. Dua
 * pembaca yang menafsirkan sendiri bentuk `INSERT INTO template` adalah persis
 * bentuk kegagalan yang uji ini ada untuk mencegahnya.
 */
describe('TRIGGER_SOURCE', () => {
  const dariMigrasi = kodePemicuDariMigrasi(AKAR);
