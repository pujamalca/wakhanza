 ...
 */
export async function summarizeSegment(rows: PatientSegmentRow[]): Promise<SegmentSummary> {
  ...
  rows.forEach((row, i) => {
    ...
    if (preview.length < PREVIEW_LIMIT) preview.push({ row, phoneE164, safe: privacy.safe });
  });
