...
export interface TargetRow {
  ...
  label: string;
  isActive: boolean;
  /** Menerima pemberitahuan pembatalan Mobile JKN -- memuat data pasien. */
  terimaBatal: boolean;
  /** Menerima SALINAN pengingat kontrol yang dikirim ke pasien. */
  terimaKontrol: boolean;
}
...
export function TargetTable({ targets, grup, waSiap }: { targets: TargetRow[]; grup: GrupRow[]; waSiap: boolean }) {
    ...
    <>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          Tujuan yang <span className="font-medium text-foreground">Nonaktif</span> tidak menerima apa pun, apa pun
          centangnya.
        </p>
      ...
      {/* Dinaikkan ke halaman, bukan ditinggal di dalam modal: "Kirim uji" dan
          "Nonaktifkan" tidak mengubah apa pun yang tampak, jadi tindakan yang
          berhasil tanpa pesan terbaca persis seperti yang gagal diam-diam. */}
      ...
      {targets.length === 0 ? (
        ...
        >
          Tanpa tujuan, pemberitahuan pembatalan tidak dikirim ke mana pun — walau sakelarnya menyala.
        </EmptyState>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className={theadClass}>
              <tr>
                ...
                <th className={`${cellClass} hidden lg:table-cell`}>Terima pembatalan</th>
                <th className={`${cellClass} hidden xl:table-cell`}>Terima salinan kontrol</th>
            ...
            <tbody>
              {targets.map((t) => (
                <tr key={t.id} className={rowClass}>
                  ...
                  {/* Dua kolom TERPISAH, bukan satu. Pembatalan memuat data
                      pasien dan gunanya supaya slot yang batal bisa ditawarkan
                      ke pasien lain; salinan kontrol adalah pesan yang juga
                      diterima pasiennya sendiri. Grup pendaftaran sangat wajar
                      perlu yang pertama tanpa yang kedua. */}
                  <td className={`${cellClass} hidden lg:table-cell`}>
                    <label className="flex cursor-pointer items-center gap-2">
                      <input
                        type="checkbox"
                        checked={t.terimaBatal}
                        disabled={pending}
                        onChange={() => jalankan(() => toggleTerimaAction(t.id, 'terimaBatal', !t.terimaBatal))}
                      />
                      <span className="text-xs text-muted-foreground">{t.terimaBatal ? 'ya' : 'tidak'}</span>
                    </label>
                  </td>
                  <td className={`${cellClass} hidden xl:table-cell`}>
                    <label className="flex cursor-pointer items-center gap-2">
                      <input
                        type="checkbox"
                        checked={t.terimaKontrol}
                        disabled={pending}
                        onChange={() => jalankan(() => toggleTerimaAction(t.id, 'terimaKontrol', !t.terimaKontrol))}
                      />
                      <span className="text-xs text-muted-foreground">{t.terimaKontrol ? 'ya' : 'tidak'}</span>
                    </label>
                  </td>
                  <td className={cellClass}>
      ...
      <ConfirmDialog
        ...
        message={
          <>
            ...
            <span className="font-medium text-foreground">tidak menerima pemberitahuan BPJS lagi</span>. Kalau cuma ingin
    ...
    <Modal
      ...
      description={
        target
          ...
          : 'Ke mana pemberitahuan BPJS dikirim.'
      }
    >
      <form action={formAction} className="space-y-3">
        <label className="block space-y-1">
          ...
          <Input
            ...
            placeholder="mis. Grup Pendaftaran"
            className="w-full"
            fieldSize="sm"
            autoFocus
          />
          ...
          <>
            ...
            {jenis === 'grup' ? (
              ...
              <label className="block space-y-1">
                ...
                />
                <span className="block text-xs text-muted-foreground">
                  Nomor pribadi petugas. Ia akan menerima data pasien di WhatsApp pribadinya — pastikan itu memang yang
                  dikehendaki.
                </span>
              </label>
            )}
